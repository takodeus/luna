const layers = [
  ...
  },
  {
    ...
    color: "var(--blue)",
    bg: "#f0f4fb",
    desc: "Governed definitions for every metric: what 'occupancy' means, which leases count, what the denominator is, who can change the rule and when. When Virtuoso computes a portfolio occupancy, it pulls the governed formula — not a guess. Every user, every agent, every report gets the same number from the same logic.",
  ...
  },
  {
    ...
    desc: "The chain of events and relationships behind every metric. When renewal rates drop 12%, the context graph connects that number to the property manager transition eight months ago, the deferred maintenance backlog, and the competitor incentive campaign in the submarket. Virtuoso stops reporting a number and starts explaining what drove it.",
...
const KnowledgeGraphSlide = () => (
  <section className="slide" id="s5">
    ...
    <p className="luna-sub" style={{ marginBottom: 0 }}>
      Cherre's knowledge graph is the infrastructure between Yardi's data and Virtuoso's intelligence. Each layer handles a distinct problem. All four are required before agent outputs can be trusted.
    </p>
