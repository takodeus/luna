...
const ClosingSlide = () => (
  <section className="slide slide-dark" id="s13">
    ...
    <h2 className="luna-h2" style={{ color: "#f0ebf3" }}>
      ...
      <br />
      <span style={{ color: "hsl(var(--pink-mid))" }}>The window to own it is open.</span>
    </h2>

    <div style={{ display: "flex", flexDirection: "column", gap: "1.5px", marginTop: "2.5rem", maxWidth: 900 }}>
      {findings.map((f, i) => (
        <div key={i} style={{ background: "hsl(var(--bg-dark))", border: "1px solid rgba(255,255,255,0.06)", borderLeft: "4px solid hsl(var(--pink))", padding: "1.8rem 2rem" }}>
          <div className="font-mono-luna" style={{ fontSize: "0.56rem", letterSpacing: "0.18em", textTransform: "uppercase", color: "hsl(var(--pink))", marginBottom: "0.6rem", opacity: 0.8 }}>{f.source}</div>
          <div className="font-serif-luna" style={{ fontSize: "1.05rem", fontWeight: 600, color: "#f0ebf3", lineHeight: 1.4, marginBottom: "0.9rem", fontStyle: "italic" }}>
            "{f.quote}"
          </div>
          <div style={{ fontSize: "0.83rem", color: "rgba(240,235,243,0.55)", lineHeight: 1.7 }}>{f.context}</div>
        </div>
      ))}
    </div>

    <div style={{ marginTop: "2.5rem", maxWidth: 900, padding: "1.6rem 2rem", background: "hsl(var(--pink))", borderRadius: 3 }}>
      <div className="font-mono-luna" style={{ fontSize: "0.56rem", letterSpacing: "0.18em", textTransform: "uppercase", color: "rgba(255,255,255,0.5)", marginBottom: "0.6rem" }}>The Strategic Position</div>
      <div style={{ fontSize: "0.95rem", color: "rgba(255,255,255,0.92)", lineHeight: 1.65, fontWeight: 400 }}>
    ...
    <div className="font-mono-luna" style={{ marginTop: "2.5rem", fontSize: "0.58rem", letterSpacing: "0.18em", textTransform: "uppercase", color: "rgba(240,235,243,0.2)" }}>
