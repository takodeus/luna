...
const GovernanceSlide = () => (
  <section className="slide" id="s9">
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1px", background: "hsl(var(--pink-border))", border: "1px solid hsl(var(--pink-border))", marginTop: "2.5rem", maxWidth: 900 }}>
      {quadrants.map((q, i) => (
        <div key={i} style={{ display: "flex", flexDirection: "column" }}>
          <div style={{ background: "hsl(var(--pink))", padding: "0.75rem 1.5rem" }}>
            <div style={{ fontSize: "0.62rem", letterSpacing: "0.14em", textTransform: "uppercase", color: "#fff", fontWeight: 600 }}>{q.label}</div>
          </div>
          <div style={{ background: "#fff", padding: "1.4rem 1.5rem", flex: 1 }}>
            <ul style={{ margin: 0, padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: "0.7rem" }}>
              {q.items.map((item, j) => (
                <li key={j} style={{ fontSize: "0.83rem", color: "hsl(var(--foreground))", lineHeight: 1.6, paddingLeft: "0.9rem", position: "relative" }}>
                  <span style={{ position: "absolute", left: 0, color: "hsl(var(--pink))", fontWeight: 700 }}>·</span>
                  {item}
                </li>
              ))}
            </ul>
